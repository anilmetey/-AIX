    ...
    <div class="w-full max-w-md mx-auto">
        ...
        <div class="bg-[#121a22]/80 backdrop-blur-xl border border-white/10 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden">
            <div class="px-8 py-10">
                <h2 class="text-2xl font-bold text-white text-center mb-2">Hoş Geldiniz</h2>
                <p class="text-sm text-gray-400 text-center font-light mb-8">Devam etmek için hesabınıza giriş yapın</p>
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-6">
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            ...
                            @if (Route::has('password.request'))
                                <a class="text-xs font-medium text-cyan-400 hover:text-cyan-300 transition-colors" wire:navigate href="{{ route('password.request') }}">
                                    Şifremi Unuttum
                                </a>
                            @endif
                        </div>
                    ...
                    <div class="flex items-center">
                        <input id="remember_me" type="checkbox" class="h-4 w-4 rounded border-white/20 bg-white/5 text-cyan-500 focus:ring-cyan-500 focus:ring-offset-gray-900" name="remember">
            ...
            @if (Route::has('register'))
                <div class="px-8 py-5 border-t border-white/5 bg-black/20 text-center">
                    <p class="text-sm text-gray-400">
                        Hesabınız yok mu? 
                        <a href="{{ route('register') }}" wire:navigate class="font-bold text-cyan-400 hover:text-cyan-300 transition-colors">Hemen Kaydolun</a>
                    </p>
                </div>
            @endif
        </div>
